...
from commons import get_rx

def _fdot(a, b):
    """Dot product for large arrays, faster than numarrays dot
    depending on the spec of the computer being used"""
    product = np.zeros((a.shape[0], b.shape[1]), 'd')
    r = 0
    while r < a.shape[0]:
        product[r, :] = np.sum(np.reshape(a[r, :], (a.shape[1], 1)) * b, 0)
        r = r + 1
    return product
...
def _mean(a, axis=0):
    ...
    default axis is 0
    """
    return np.sum(a, axis) / a.shape[axis]
...
def _std(a):
    ...
    """
    m = _mean(a, 0)
    m = np.resize(m, (a.shape[0], a.shape[1]))
    return np.sqrt(np.sum((a-m)**2, 0) / (a.shape[0] - 1))

def _diag(a):
    """Transform vector to diagonal matrix
    """
    d = np.zeros((len(a), len(a)), 'd')
...
def _rms(pred, act):
    ...
    """
    return np.reshape(np.sqrt(np.sum((act - pred)**2) / act.shape[0]), ())
    
def _min(x, axis=0):
    """find min of 2d array x along axis 0 or 1
    """
    s = np.sort(x, axis)
    return np.reshape(s[0], ())

def _max(x, axis=0):
    """find min of 2d array x along axis 0 or 1
    """
    s = np.sort(x, axis)
    return np.reshape(s[x.shape[0] - 1], ())
...
def _split(xdata, ydata, mask, labels=None):
    ...
    """
    x1 = np.take(xdata, _index(mask, 0), 0)
    x2 = np.take(xdata, _index(mask, 1), 0)
    y1 = np.take(ydata, _index(mask, 0), 0)
    y2 = np.take(ydata, _index(mask, 1), 0)
    n1, n2 = [], []
    ...
    if max(mask) == 1:
        ...
        x3 = np.take(xdata, _index(mask, 2), 0)
        y3 = np.take(ydata, _index(mask, 2), 0)
        n3 = []
        if labels is not None:
            for i in range(len(labels)):
                if mask[i] == 2:
...
def pca_svd(myarray, typex='covar'):
    ...
    # mymat = np.mat(myarray)
    u, s, v = sp.linalg.svd(myarray)
    tt = np.dot(myarray, np.transpose(v))
    pp = v
    pr = (1 - (s / np.sum(np.sum(myarray**2)))) * 100
    pr = np.reshape(pr, (1, len(pr)))
    pr = np.concatenate((np.array([[0.0]]), pr), 1)
...
# noinspection PyShadowingNames
def pca_nipals(myarray, comps, typex='covar', stb=None):
    ...
    tt, pp, i = np.zeros((arr_size[0], comps), 'd'), np.zeros((comps, arr_size[1]), 'd'), 0
        
    while i < comps:
        std = np.std(newarray, axis=0)
        st2 = np.argsort(std)
        ind = st2[arr_size[1]-1, ]
        t0 = newarray[:, ind]
        c = 0
        while c == 0:   # NIPALS
            p0 = np.dot(np.transpose(t0), newarray)
            p1 = p0/np.sqrt(np.sum(p0**2))
            t1 = np.dot(newarray, np.transpose(p1))
            if np.sqrt(np.sum(t1**2)) - np.sqrt(np.sum(t0**2)) < 5*10**-5:
                tt[:, i] = t1
                pp[i, :] = p1
                c = 1
            t0 = t1

        newarray = newarray - np.dot(np.resize(t1, (arr_size[0], 1)),
                                     np.resize(p1, (1, arr_size[1])))
    ...
    s0, s = np.sum(np.sum(newarray**2)), []
    for n in np.arange(1, comps+1, 1):
        E = newarray - np.dot(tt[:, 0:n], pp[0:n, :])
        s.append(np.sum(np.sum(E**2)))
        
    pr = (1-(np.asarray(s) / s0)) * 100
    pr = np.reshape(pr, (1, len(pr)))
    pr = np.concatenate((np.array([[0.0]]), pr), 1)
    pr = np.reshape(pr, (pr.shape[1], ))
    eigs = np.array(s)
...
def cva(X, group, nodfs, pcloads=None):
    ...
    >>> import numpy as np
    >>> X = np.array([[ 0.19343116, 0.49655245, 0.72711322, 0.79482108, 0.13651874],
    ...
    if pcloads is not None:
        pcloads = np.dot(np.transpose(pcloads), As_out)
...
def mlr(x, y, order):
    ...
    if order > 0:
        ...
        for j in range(order):
            s = np.concatenate((s, (np.arange(0, 1 + (1 / (len(y)-1)),
                                              1 / (len(y)-1))**j)[:, nax]), 1)
    ...
    b = np.dot(np.dot(sp.linalg.pinv(np.dot(np.transpose(X), X)),
                      np.transpose(X)), y)
    f = np.dot(X, b)
    r = y - f
...
# noinspection PyShadowingNames
def pls(xdata, ydata, mask, factors, stb=None, typex=0):
    ...
    >>> import numpy as np
    >>> xdata = np.array([[ 0.6 , 0.57, 0.59, 0.81, 0.45], [ 0.96, 0.76, 0.99, 0.08, 0.17],
    ...                   [ 0.99, 0.06, 0.99, 0.28, 0.98], [ 0.2 , 0.02, 0.35, 0.12, 0.34],
    ...                   [ 0.36, 0.02, 0.02, 0.48, 0.07], [ 0.5 , 0.5 , 0.59, 0.26, 0.81],
    ...                   [ 0.71, 0.07, 0.37, 0.09, 0.53], [ 0.2 , 0.88, 0.48, 0.53, 0.93],
    ...                   [ 0.63, 0.44, 0.47, 0.33, 0.02], [ 0.63, 0.45, 0.23, 0.5 , 0.59]])
    >>> ydata = np.array([[ 0.48], [ 0.55], [ 0.74], [ 0.43], [ 0.52], [ 0.95], [ 0.7 ], [ 0.23], [ 0.08], [ 0.42]])
    >>> mask = np.array([[0], [2], [0], [1], [0], [1], [1], [0], [2], [0]])

    >>> output = pls(xdata, ydata, mask, 3)
    >>> # output.keys => W, T, P, Q, facs, predy, predyv, predyt, RMSEC, RMSEPC, rmsec, rmsepc, RMSEPT, b
    >>> output['W']
    ...
    """
    w, t, p, q = None, None, None, None
    Xmt, ymt = None, None
    output = {}
    # raw data
    x1, x2, x3, y1, y2, y3, _, _, _ = _split(xdata, ydata, mask)
    # get column means
    Xm, Xmv = np.mean(x1, axis=0), np.mean(x2, axis=0)
    ym, ymv = np.mean(y1, axis=0), np.mean(y2, axis=0)
    
    if max(mask) > 1:
        Xmt, ymt = np.mean(x3, axis=0), np.mean(y3, axis=0)
    
    if typex == 0:
        # use matrix of covariances
        x, y = _meancent(xdata), _meancent(ydata)
    else:
        # use matrix of correlations
        x, y = _autoscale(xdata), _autoscale(ydata) 
    
    # split into training, cross-validation & test
    train_x, cval_x, test_x, train_y, cval_y, test_y, _, _, _ = _split(x, y, mask)
    ...
    for xi in range(factors):
        ...
        if NoY > 1:
            # PLS2
            u = u[:, np.argsort(np.sum(y1**2, axis=0))[0]][:, np.newaxis]
        
        while opt == 0:
            # for training
            c = np.dot(np.dot(np.dot(np.transpose(u), X), np.transpose(X)), u)**-0.5  # scaling factor
            w = np.transpose(c * np.dot(np.transpose(u), X))  # vector of loading weights, w'w = 1
            t = np.dot(X, w)     # spectral scores
            p = np.transpose(np.dot(np.transpose(X), t) * sp.linalg.inv(np.dot(np.transpose(t), t)))  # spectral loadings
            q = np.dot(np.transpose(t), y) * sp.linalg.inv(np.dot(np.transpose(t), t))  # chemical loading
            
            if NoY == 1:
                # PLS1
                opt = 1                
            elif float(np.reshape(np.sum(abs(t-t0)), ())) > 5*10**-5:
                # PLS2 - check for convergence
                u = np.dot(np.dot(y, np.transpose(q)), sp.linalg.inv(np.dot(q, np.transpose(q))))
                t0 = t
            else:
                opt = 1
        # compute residuals of X which are also X for next iteration
        X = X - np.dot(t, p)
        # compute residuals of y which are also y for next iteration
        u = u - np.dot(t, q)
        
        if xi == 0:
            output['W'], output['T'], output['P'], output['Q'] = w, t, p, q
        else:
            output['W'] = np.concatenate((output['W'], w), 1)
            output['T'] = np.concatenate((output['T'], t), 1)
            output['P'] = np.concatenate((output['P'], p), 0)
            output['Q'] = np.concatenate((output['Q'], q), 0)
        
        b = np.dot(np.transpose(output['Q']), np.transpose(np.dot(output['W'],
                   sp.linalg.inv(np.dot(output['P'], output['W'])))))
        
        output['spec'] = b
        
        # rms for training data - rmsec
        b0 = ym - np.dot(Xm[np.newaxis, :], np.transpose(b))
        predy = b0 + np.dot(x1, np.transpose(b))
        output['rmsec'].append(float(_rms(predy, y1)))
            
        # cross validation prediction
        b1 = ymv - np.dot(Xmv[np.newaxis, :], np.transpose(b))
        predyv = b1 + np.dot(x2, np.transpose(b))
        output['rmsepc'].append(float(_rms(predyv, y2)))
        
        # independent test predictions
        if max(mask) > 1:
            b2 = ymt - np.dot(Xmt[np.newaxis, :], np.transpose(b))
            predyt = b2 + np.dot(x3, np.transpose(b))
            output['rmsept'].append(float(_rms(predyt, y3)))
            
        # report progress to status bar
        if stb is not None:
            msg = 'Extracting factor...%i ' % (xi+1, )
            stb.SetStatusText(msg, 0)

    # work out number of factors to use by finding the min of
    # the rmsep cross validation - exception for use in GA
    output['facs'] = ind = np.argsort(output['rmsepc'])[0]
        
    # return final rms values
    output['RMSEC'] = output['rmsec'][ind]
    output['RMSEPC'] = output['rmsepc'][ind]

    # claculate partial prediction based on optimal no. of factors
    output['b'] = np.dot(np.transpose(output['Q'][0:ind+1, :]),
                         np.transpose(np.dot(
                             output['W'][:, 0:ind+1],
                             sp.linalg.inv(np.dot(output['P'][0:ind+1, :],
                                           output['W'][:, 0:ind+1])))))
    
    # predictions based on optimal no. of factors
    # training predictions
    b0 = ym - np.dot(Xm[np.newaxis, :], np.transpose(output['b']))
    predy = b0 + np.dot(x1, np.transpose(output['b']))
    output['RMSEC'] = float(_rms(predy, y1))
        
    # cross validation prediction
    b1 = ymv - np.dot(Xmv[np.newaxis, :], np.transpose(output['b']))
    predyv = b1 + np.dot(x2, np.transpose(output['b']))
    output['RMSEPC'] = float(_rms(predyv, y2))
    
    # independent test predictions
    if max(mask) > 1:
        b2 = ymt - np.dot(Xmt[np.newaxis, :], np.transpose(output['b']))
        predyt = b2 + np.dot(x3, np.transpose(output['b']))
    ...
    output['predictions'] = np.zeros(ydata.shape)
    output['predictions'] = _put(output['predictions'], _index(mask, 0), predy[:, np.newaxis])
    output['predictions'] = _put(output['predictions'], _index(mask, 1), predyv[:, np.newaxis])
    if max(mask) > 1:
        output['predictions'] = _put(output['predictions'], _index(mask, 2), predyt[:, np.newaxis])
    
    # recalculate spectral scores for all samples
    output['plsscores'] = np.dot(x, output['W'][:, 0:ind+1])
...
def dfa_xval_raw(X, group, mask, nodfs):
    ...
    x1, x2, x3, y1, y2, y3, _, _, _ = _split(X, np.array(group, 'i'), mask)
    
    # get indices
    idxn = np.arange(X.shape[0])[:, nax]
    tr_idx = np.take(idxn, _index(mask, 0), 0)
    cv_idx = np.take(idxn, _index(mask, 1), 0)
    
    # train
    trscores, loads, eigs, loads2 = cva(x1, y1, nodfs)
    
    # cross validation
    cvscores = np.dot(x2, loads)
    
    # independent test
    if max(mask) > 1:
        ts_idx = np.take(idxn, _index(mask, 2), 0)
        tstscores = np.dot(x3, loads)
        
        scores = np.zeros((X.shape[0], nodfs), 'd')
        
        tr_idx = np.reshape(tr_idx, (len(tr_idx), )).tolist()
        cv_idx = np.reshape(cv_idx, (len(cv_idx), )).tolist()
        ts_idx = np.reshape(ts_idx, (len(ts_idx), )).tolist()
        _put(scores, tr_idx, trscores)
        _put(scores, cv_idx, cvscores)
        _put(scores, ts_idx, tstscores)
        
    else:
        scores = np.concatenate((trscores, cvscores), 0)
        tr_idx = np.reshape(tr_idx, (len(tr_idx), )).tolist()
        cv_idx = np.reshape(cv_idx, (len(cv_idx), )).tolist()
        _put(scores, tr_idx, trscores)
        _put(scores, cv_idx, cvscores)
...
def dfa_xval_pca(X, pca, nopcs, group, mask, nodfs, ptype='covar'):
    ...
    """
    pcscores, pp = None, None

    rx1, rx2, rx3, ry1, ry2, ry3, dummy1, dummy2, dummy3 =\
        _split(X, np.array(group, 'i')[:,  nax],  mask[:,  nax])
    
    if pca == 'SVD':
        pcscores, pp, pr, pceigs = pca_svd(rx1, typex=ptype)
    elif pca == 'NIPALS':
        pcscores, pp, pr, pceigs = pca_nipals(rx1, nopcs, typex=ptype)
    ...
    rx2 = get_rx(rx1, rx2, ptype)

    pcscores = np.dot(rx2, np.transpose(pp))
    cvscores = np.dot(pcscores[:, 0:nopcs], loads)
    
    # independent test
    if max(mask) > 1:
        ts_idx = np.take(idxn, _index(mask, 2), 0)
        rx3 = get_rx(rx1, rx3, ptype)

        pcscores = np.dot(rx3, np.transpose(pp))
        tstscores = np.dot(pcscores[:, 0:nopcs], loads)
    ...
    loads = np.dot(np.transpose(pp[0:nopcs, :]), loads)
...
def dfa_xval_pls(plsscores, plsloads, nolvs, group, mask, nodfs):
    ...
    """
    rx1, rx2, rx3, ry1, ry2, ry3, dummy1, dummy2, dummy3 = \
        _split(plsscores, np.array(group, 'i')[:, nax], mask[:, nax])
    
    # get indices
    idxn = np.arange(plsscores.shape[0])[:, nax]
    tr_idx = np.take(idxn, _index(mask, 0), 0)
    cv_idx = np.take(idxn, _index(mask, 1), 0)
    ...
    # cross validation
    cvav = np.dot(rx2[:, 0:nolvs], loads)
    
    # independent test
    if max(mask) > 1:
        cvat = np.dot(rx3[:, 0:nolvs], loads)
        
        scores = np.zeros((plsscores.shape[0], nodfs), 'd')
        ...
        scores = np.concatenate((cvas, cvav), 0)
    ...
    # get loadings for original variables
    loads = np.dot(plsloads[:, 0:nolvs], loads)
...
def ols(act, pred):
    ...
    gradient = np.sum((act-_mean(act))*(pred-_mean(pred)))/(sum((act-_mean(act))**2))
    yintercept = _mean(act)-(gradient*_mean(act))
    mserr = pred-_mean(pred)
    
    rmserr = _rms(act, pred)
    gerr = np.sqrt(rmserr**2 / np.sum((act-_mean(act))**2))
    ierr = np.sqrt((rmserr**2) * ((1/len(act)) +
                                  ((_mean(act)**2) / np.sum((act - _mean(act))**2))))

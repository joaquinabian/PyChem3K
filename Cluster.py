...
class Cluster(wx.Panel):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.plcCluster = MyPlotCanvas(
            id=-1, name='plcCluster', parent=self.p1, pos=wx.Point(0, 0),
            size=wx.Size(200, 200), style=wx.SUNKEN_BORDER,
        ...
        self.plcCluster.SetConstraints(LayoutAnchors(self.plcCluster, True, True,
              True, True))
        ...
        self.txtCluster.SetConstraints(LayoutAnchors(self.txtCluster, True, True,
              True, True))
...
class TitleBar(bp.ButtonPanel):
    def _init_btnpanel_ctrls(self, prnt):
        ...
        self.cbxData = wx.Choice(choices=['Raw spectra','Processed spectra','PC Scores','DF Scores'], id=-1,
              name='cbxData', parent=self, pos=wx.Point(118, 21),
              size=wx.Size(100, 23), style=0)
        self.cbxData.SetSelection(0)
        
        self.btnRunClustering = bp.ButtonInfo(self, -1, wx.Bitmap(os.path.join('bmp','run.png'),wx.BITMAP_TYPE_PNG), 
                                kind=wx.ITEM_NORMAL,
        ...
        self.btnExportCluster = bp.ButtonInfo(self, -1, wx.Bitmap(os.path.join('bmp','export.png'),wx.BITMAP_TYPE_PNG), 
                                kind=wx.ITEM_NORMAL,shortHelp='Export Cluster Results',longHelp='Export Cluster Results')
        self.btnExportCluster.Enable(False)
        self.Bind(wx.EVT_BUTTON, self.OnBtnExportClusterButton, id=self.btnExportCluster.GetId())
        
        self.setParams = bp.ButtonInfo(self, -1, wx.Bitmap(os.path.join('bmp','params.png'),wx.BITMAP_TYPE_PNG), 
                                kind=wx.ITEM_NORMAL,shortHelp='Clustering Options',longHelp='Clustering Options')
        ...
        try:
            ...
            if self.parent.optDlg.rbKmeans.GetValue() is True:
                if self.parent.optDlg.spnNoPass.GetValue() == 1:
                    stid = (sp.array(self.data['class'][:,0],'i')-1).tolist()
                    self.clusterid, error, nfound = cluster.kcluster(xdata, 
                          nclusters=len(sp.unique(self.data['class'])), 
                          transpose=0, npass=1, method='a', dist=seldist, 
                          initialid=stid)
                else:
                    self.clusterid, error, nfound = cluster.kcluster(xdata, 
                          nclusters=len(sp.unique(self.data['class'][:,0])),
                          transpose=0, npass=self.parent.optDlg.spnNoPass.GetValue(), 
                          method='a', dist=seldist)
                                                                
                self.parent.plcCluster.Show(False)
                self.parent.txtCluster.Show(True)
                
                centroids,mask = cluster.clustercentroids(xdata, 
                      clusterid=self.clusterid, method='a',transpose=0)
        
                self.ReportPartitioning(self.parent.txtCluster,self.clusterid,
                      error,nfound,'K-means Summary',centroids)
                ...
                if self.parent.optDlg.spnNoPass.GetValue() == 1:
                    ...
                    self.clusterid, error, nfound = cluster.kcluster(xdata, 
                          nclusters=len(sp.unique(self.data['class'][:,0])),
                          npass=1, method='m', dist=seldist, initialid=stid)
                else:
                    self.clusterid, error, nfound = cluster.kcluster(xdata, 
                          nclusters=len(sp.unique(self.data['class'][:,0])),
                          npass=self.parent.optDlg.spnNoPass.GetValue(), 
                          method='m', dist=seldist)
                
                self.parent.plcCluster.Show(False)
                self.parent.txtCluster.Show(True)
                
                centroids, mask = cluster.clustercentroids(
                    xdata, mask=ones(xdata.shape),
                    clusterid=self.clusterid, method='m', transpose=0)
                
                self.ReportPartitioning(self.parent.txtCluster,self.clusterid,
                      error,nfound,'K-medians Summary',centroids)
            
            elif self.parent.optDlg.rbKmedoids.GetValue() is True:
                # generate distance matrix
                distance = cluster.distancematrix(
                    xdata, transpose=0, dist=seldist)
                
                if self.parent.optDlg.spnNoPass.GetValue() == 1:
                    stid = (np.array(self.data['class'][:,0],'i')-1).tolist()
                    self.clusterid, error, nfound = cluster.kmedoids(distance, 
                          nclusters=len(sp.unique(self.data['class'][:,0])), 
                ...
                #run hca
                tree = cluster.treecluster(data=xdata, method=Hmeth, dist=seldist)
                
                #scale tree
                tree.scale()
                
                #divide into clusters
##                tree.cut(len(sp.unique(self.data['class'][:,0])))
                ...
                #draw tree
                self.drawTree(self.parent.plcCluster, self.data['tree'], 
    ...
    def treestructure(self, tree, order):
        ...
        nodeid = sp.zeros((nnodes,4),'d')
        nodecounts = sp.zeros(nnodes)
        nodeorder = sp.zeros(nnodes,'d')
        nodedist = sp.array(nodedist)
...
##                #plot names
##                if idn in List:
##                    Store[str(idn)] = sp.concatenate((Store[str(idn)],[[0,count]]),0)
##                else:
...
##        elif self.parent.optDlg.rbPlotName.GetValue() is True:
        ...
        count = 0
        for i in range(len(order)):
            Line.append(wx.lib.plot.PolyMarker(sp.array([[0,count],[0,count]]),
                  marker='text', labels=[labels[int(order[i])],labels[int(order[i])]]))
            count += 2
                
        #plot distances
        Line.append(wx.lib.plot.PolyMarker(sp.array([[0,-2]]),marker='text',
                                    labels='0'))
        Line.append(wx.lib.plot.PolyMarker(sp.array([[max(tree[:,3]),-2]]),
                                    marker='text',labels='% .2f' %max(tree[:,3])))
        
        idx = sp.reshape(sp.arange(len(tree)+1),(len(tree)+1,))
        Nodes = {}
        for i in range(len(tree)):
            #just samples    
            if tree[i,1] >= 0:
                if tree[i,2] >= 0:
                    ...
                    for iix in range(len(pos)):
                        if pos[iix] == 1:
                            y1 = iix*2
                    Line.append(wx.lib.plot.PolyLine(sp.array([[x1,y1],[x2,y1]]),colour='black',
                    ...
                    for iix in range(len(pos)):
                        if pos[iix] == 1:
                            y2 = iix*2
                    Line.append(wx.lib.plot.PolyLine(sp.array([[x1,y2],[x2,y2]]),colour='black',
                                                width=1.5, style=wx.SOLID))
                    #connect
                    Line.append(wx.lib.plot.PolyLine(sp.array([[x2,y1],[x2,y2]]),colour='black',
                                                width=1.5, style=wx.SOLID))
                    #save node coord
    ...
    def ReportPartitioning(self,textctrl,clusterid,error,nfound,title,centroids=None):
        ...
        confarr = sp.zeros((int(max(self.data['class'][:,0])),int(max(self.data['class'][:,0]))))
...
class selFun(fpb.FoldPanelBar):
    ...
    def _init_coll_gbsLinkageMethod_Items(self, parent):
        parent.Add(self.rbSingleLink, (0, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbMaxLink, (1, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbAvLink, (2, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbCentLink, (3, 0), flag=wx.EXPAND,
              span=(1, 1))
        # parent.AddSpacer(8)
        # parent.AddSpacer(wx.Size(8, 8), (5, 0), flag=wx.EXPAND, span=(2, 2))
    
    def _init_coll_gbsDistanceMeasure_Items(self, parent):
        parent.Add(self.rbAbsUncentCorr, (0, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbAbsCorr, (1, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbCityBlock, (2, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbCorrelation, (3, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbEuclidean, (4, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbKendalls, (5, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbSpearmans, (6, 0), flag=wx.EXPAND,
              span=(1, 1))
        parent.Add(self.rbUncentredCorr, (7, 0), flag=wx.EXPAND,
              span=(1, 1))
        # parent.AddSpacer(8)
        # parent.AddSpacer(wx.Size(8, 8), (5, 0), flag=wx.EXPAND, span=(2, 2))
    ...
    def _init_selfun_ctrls(self, prnt):
        ...
        self.rbKmeans = wx.RadioButton(id=-1, label='k-means clustering',
              name='rbKmeans', parent=self.methPnl, pos=wx.Point(16, 48),
        ...
        self.rbKmedian = wx.RadioButton(id=-1, label='k-medians clustering',
              name='rbKmedian', parent=self.methPnl, pos=wx.Point(16, 24),
        ...
        self.rbKmedoids = wx.RadioButton(id=-1, label='k-medoids clustering',
              name='rbKmedoids', parent=self.methPnl, pos=wx.Point(16, 72),
        ...
        self.rbHcluster = wx.RadioButton(id=-1, label='Hierarchical clustering',
              name='rbHcluster', parent=self.methPnl, pos=wx.Point(16, 96),
        ...
        self.rbSingleLink = wx.RadioButton(id=-1, label='Single linkage',
              name='rbSingleLink', parent=self.linkPnl, pos=wx.Point(168, 144),
              size=wx.Size(88, 21), style=wx.RB_GROUP)
        self.rbSingleLink.SetValue(True)
        self.rbSingleLink.SetToolTip('')
        
        self.rbMaxLink = wx.RadioButton(id=-1, label='Maximum linkage',
              name='rbMaxLink', parent=self.linkPnl, pos=wx.Point(40, 144),
        ...
        self.rbAvLink = wx.RadioButton(id=-1, label='Average linkage',
              name='rbAvLink', parent=self.linkPnl, pos=wx.Point(40, 120),
        ...
        self.rbAbsCorr = wx.RadioButton(id=-1,
              label='Absolute value of correlation', name='rbAbsCorr',
              parent=self.distPnl, pos=wx.Point(16, 232), size=wx.Size(184, 21),
              style=0)
        self.rbAbsCorr.SetValue(False)
        self.rbAbsCorr.SetToolTip('')
        
        self.rbUncentredCorr = wx.RadioButton(id=-1,
              label='Uncentred correlation', name='rbUncentredCorr',
              parent=self.distPnl, pos=wx.Point(16, 400), size=wx.Size(136, 21),
              style=0)
        self.rbUncentredCorr.SetValue(False)
        self.rbUncentredCorr.SetToolTip('')
        
        self.rbAbsUncentCorr = wx.RadioButton(id=-1,
              label='Absolute uncentred correlation', name='rbAbsUncentCorr',
              parent=self.distPnl, pos=wx.Point(16, 208), size=wx.Size(176, 21),
              style=0)
        self.rbAbsUncentCorr.SetValue(False)
        self.rbAbsUncentCorr.SetToolTip('')
        
        self.rbSpearmans = wx.RadioButton(id=-1,
              label='Spearmans rank correlation', name='rbSpearmans',
              parent=self.distPnl, pos=wx.Point(16, 376), size=wx.Size(168, 21),
              style=0)
        self.rbSpearmans.SetValue(False)
        self.rbSpearmans.SetToolTip('')
        
        self.rbKendalls = wx.RadioButton(id=-1, label='Kendalls rho',
              name='rbKendalls', parent=self.distPnl, pos=wx.Point(16, 352),
              size=wx.Size(136, 21), style=0)
        self.rbKendalls.SetValue(False)
        self.rbKendalls.SetToolTip('')
        
        self.rbCityBlock = wx.RadioButton(id=-1, label='City-block distance',
              name='rbCityBlock', parent=self.distPnl, pos=wx.Point(16, 256),
              size=wx.Size(136, 21), style=0)
...
##        self.cbUseClass.Bind(wx.EVT_CHECKBOX, self.OnCbUseClassCheckbox, id_=-1)
        
        self.spnNoPass = wx.SpinCtrl(id=-1, initial=1, max=1000, min=1,
              name='spnNoPass', parent=self.methPnl, pos=wx.Point(200, 444),
              size=wx.Size(80, 23), style=wx.SP_ARROW_KEYS)
        ...
        self.spnNumClass= wx.SpinCtrl(id=-1, initial=1, max=1000, min=1,
              name='spnNumClass', parent=self.methPnl, pos=wx.Point(200, 444),
              size=wx.Size(80, 23), style=wx.SP_ARROW_KEYS)
        ...
        self.rbPlotName = wx.RadioButton(id=-1, label='Plot using labels',
              name='rbPlotName', parent=self.distPnl, pos=wx.Point(16, 480),
              size=wx.Size(104, 21), style=wx.RB_GROUP)
        ...
        self.rbPlotColours = wx.RadioButton(id=-1, label='Plot using colours',
              name='rbPlotColours', parent=self.distPnl, pos=wx.Point(144, 480),
              size=wx.Size(104, 21), style=0)
